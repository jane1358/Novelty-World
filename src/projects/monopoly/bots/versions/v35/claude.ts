...
// v34 changes valuation.ts only; dispatcher is v29 verbatim. — fork of the champion v28 (see EVOLUTION.md). The DISPATCHER and
...
// ---------------------------------------------------------------------------
// v35: STRETCH — when an opponent needs this lot for a monopoly, they'll pay
// a premium. Bid toward their valuation to drain their cash, not just ours.
const AUCTION_STRETCH = 0.85;

/** The maximum price the most motivated opponent would pay for `pos` —
 *  face value plus the monopoly bonus if this lot completes their set. Only
 *  counts opponents who are exactly one lot short (the auctioned lot is the
 *  missing piece). */
function opponentThreatValue(state: GameState, pid: string, pos: number): number {
  let threat = 0;
  const price = ownablePrice(pos);
  if (price === null) return 0;
  for (const opp of activeOpponents(state, pid)) {
    const oppValue = acquisitionValue(state, opp.id, pos);
    threat = Math.max(threat, oppValue);
  }
  return threat;
}
...
function auction(state: GameState, pid: string): BotDecision | null {
  ...
  // My own valuation (includes denial value already).
  const myValue = acquisitionValue(state, pid, a.position);

  // v35: If an opponent wants this badly (monopoly completer), stretch the
  //  bid toward their valuation to drain their cash — they either pay more
  //  or I win the blocker. But never exceed my own value by more than the
  //  stretch factor allows, so I don't overpay for something I don't want.
  const threat = opponentThreatValue(state, pid, a.position);
  const stretchCap = Math.round(threat * AUCTION_STRETCH);

  // The effective cap: max of my value and the stretched threat, but only
  // stretch if the threat exceeds my value (otherwise it's just my value).
  const effectiveValue = threat > myValue ? Math.max(myValue, stretchCap) : myValue;
  const cap = Math.min(effectiveValue, auctionBidCap(state, pid));
