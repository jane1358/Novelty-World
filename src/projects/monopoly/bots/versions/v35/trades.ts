...
// ===========================================================================
// v35 SNAPSHOT — fork of v28 (the champion; see EVOLUTION.md).
// v35 adds DISTRESS-ACCELERATED DENIAL: when the holder of a set-completer is
// distressed, they are more likely to sell to the rival at a discount, so the
// denial fires sooner (rivalCanAcquire returns true more often). — fork of v17 (the champion; see EVOLUTION.md).
...
function rivalCanAcquire(
  ...
  // v35: a distressed holder values cash above face (v28), so the rival can
  //  acquire the lot for LESS. Reduce the cost barrier by the holder distress.
  const holderDistress = sellerDistress(state, holder);
  const need = Math.ceil((ACCEPT_MARGIN - holderDelta) / (1 + holderDistress * 0.4));
