// ===========================================================================
// v35 SNAPSHOT — fork of v34 (see EVOLUTION.md).
// v35 changes claude.ts only (auction logic); valuation is v34 verbatim. — fork of the champion v29 (see EVOLUTION.md).
// v34 adds ENDGAME AGGRESSION: when only 2 active players remain, lower the
// liquidity floor and hotel cushion to deploy capital faster into development.
// In a 2-player endgame, every dollar is zero-sum — the faster developer wins
// the rent race. The distress pricing from v28/v29 is carried verbatim. — fork of the champion v28 (see EVOLUTION.md). IDENTICAL to v28
// except ONE constant: `DISTRESS_DISCOUNT` 0.75→1.0 — pushing v28's winning
// desperation-acquisition lever to its MAXIMUM (a distressed seller prices NO
// rival-monopoly threat, selling at its bare cash-equivalent break-even, so the
// buyer banks the whole premium). Everything else — the distress gate, the seller
// scale wiring, the buyer construction, v17's reserve — is v28 verbatim.
// ===========================================================================
...
export function liquidityFloor(state: GameState, pid: string): number {
  ...
  // v34: In a 2-player endgame, keep a thinner reserve — the urgency of
  //  deploying capital outweighs buffering against one opponent's rents.
  const endgame = activeOpponents(state, pid).length <= 1;
  const frac = endgame ? FLOOR_RENT_FRACTION * 0.6 : FLOOR_RENT_FRACTION;
  const cap = endgame ? Math.round(FLOOR_CAP * 0.6) : FLOOR_CAP;
  const base = endgame ? Math.round(BASE_FLOOR * 0.6) : BASE_FLOOR;
  return Math.min(cap, Math.max(base, Math.round(worst * frac)));
}

// ---------------------------------------------------------------------------
// v28 — DISTRESS pricing (the desperation-acquisition lead, lead b). A genuinely
// near-bankrupt seat values immediate cash above an asset's future strategic
// worth: facing a developed rent it can't cover, it will part with a building-free
// lot for less than its normal break-even, because that lot's premium is worth
// nothing if a single landing busts it first. This is the SELLER half of the
// coupled hypothesis — it is what lets an UNDERPRICED buy actually EXECUTE in
// self-play (a non-distressed v17 seat would just decline a below-fair offer and
// the deal would wash, exactly v24's lesson). It is VOLUNTARY/pre-emptive: it
// fires BEFORE the engine's forced must-raise-cash path (which only triggers
// after a charge already put cash negative), so the seat raises liquidity by
// choice while it still can. It is NOT the possessiveness levers that regressed
// (v9/v13/v15 made the bot HOLD MORE) — this makes a DESPERATE seat dump faster,
// which is aggression-consistent.
// ---------------------------------------------------------------------------

/** A developed rent at or above this is "deadly" — a single landing that, against
 *  a seat that can't cover it, forces liquidation or bankruptcy. Same threshold
 *  the jail-haven logic uses to call a board dangerous (~a 3-house mid-set rent). */
const DEADLY_RENT = JAIL_DANGER_RENT;

/** How far a distressed seller discounts the rival-monopoly threat premium it
 *  would normally hold out for. At 1.0 the desperate seat ignores the future
 *  threat entirely and prices the lot at its cash-equivalent break-even; the
 *  premium it forgoes is the buyer's discount.
 *
 *  v29 PUSHES the winning lever: v28's 0.75 WON, so per the loop probe the
 *  MAXIMAL discount — 1.0, the desperate seat prices NO threat at all, so the
 *  buyer banks the whole premium. The hypothesis: a seat one landing from
 *  bankruptcy gains nothing from guarding against arming a rival it may never
 *  outlive, so it should take the most cash on offer; the deeper discount lets
 *  the bot buy MORE completers (deals that didn't clear at 0.75 now do) and
 *  cheaper. Risk (the v7/v10 "push the parameter harder" trap): if 0.75 already
 *  captured the band, 1.0 just leaves cash on the table the seller would've taken
 *  anyway and/or fires on marginally-distressed seats that recover — a wash. */
const DISTRESS_DISCOUNT = 1.0;

/** The worst single DEVELOPED rent `pid` would face on a board it doesn't own —
 *  the imminent hit that can bust a thin seat. Only developed lots count: a bare
 *  lot's rent is small and survivable, so it isn't a distress trigger. */
...
/** Is `pid` GENUINELY distressed — one bad landing from bankruptcy? True only
 *  when a DEADLY developed rent is out there AND `pid` can't cover it even after
 *  liquidating everything mortgageable (`cash + mortgageableTotal < deadlyRent`).
 *  Deliberately strict: not "low-ish cash" but "a single landing ends me even if
 *  I mortgage out", so the discount fires rarely and only when desperation is
 *  real — the gate the lead demands to avoid mis-modeling the discount. */
...
/** The fraction of the rival-monopoly threat premium a SELLER actually charges
 *  for handing over a building-free lot: the full premium normally, but discounted
 *  to `1 − DISTRESS_DISCOUNT` when `pid` is genuinely distressed (it weights the
 *  cash it needs NOW above the future cost of an empowered rival). This is the one
 *  number `evaluateTrade` scales the threat by — the whole seller half. */
...
function desiredLevel(
  ...
  // v34: In a 2-player endgame, the cushion for hotel-level development is
  //  halved — the rent race is more urgent and cash reserves less important
  //  (there's only one opponent to worry about, and they're racing too).
  const endgame = activeOpponents(state, pid).length <= 1;
  const hotelCushion = endgame ? Math.round(HOTEL_CUSHION / 2) : HOTEL_CUSHION;
  const flush = cash > liquidityFloor(state, pid) + hotelCushion;
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // v34: endgame halved cushion
  const endgame = activeOpponents(state, pid).length <= 1;
  const hotelCushion = endgame ? Math.round(HOTEL_CUSHION / 2) : HOTEL_CUSHION;
  const flush = player.cash > floor + hotelCushion;
