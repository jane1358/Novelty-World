...
import type { GameState, ManageStaged, PropertyColor, TradeTerms } from "../../../types";
...
function buyDecision(state: GameState, pid: string): BotDecision | null {
  ...
  // Early-game aggression: when no one has a monopoly yet, buy everything
  // affordable. Properties are trade chips — owning more maximizes trading
  // leverage to complete sets later. Don't reserve for rents that don't exist yet.
  const anyMonopoly = COLORS.some((c) =>
    state.players.some((p) => hasMonopoly(state, c, p.id)));
  const earlyGame = !anyMonopoly;

  if (player.cash >= price) {
    if (player.cash - price >= floor || earlyGame) {
      return {
