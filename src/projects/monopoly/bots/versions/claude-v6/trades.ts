...
// ===========================================================================
// v6 SNAPSHOT — self-contained fork of v5's `trades.ts` (see EVOLUTION.md).
//
// INHERITED (v5): trade-to-deny. When a rival is one lot short of a set and the
// completer sits with a third-party holdout, buy that lot to ME purely to block
// the completion, credited the `DENY_FACTOR` premium on the rival set. The denied
// rival is not a party, so it can't veto its own denial — the negative-sum
// asymmetry that, unlike v3/v4's positive-sum self-improvements, transferred win
// share. `evaluateTrade` is unchanged; the denial credit lives only in the
// proposer's go/no-go (gate = plainDelta + DENY_FACTOR×bonus > ACCEPT_MIN).
//
// v6 HYPOTHESIS: "the proven denial wins MORE when it isn't gated by cash." v5
// could only fund the holdout's sweetener in CASH (`sweetenFor` returns null when
// the bot can't cover it), so a denial it wanted but couldn't afford simply never
// happened — and mid-game, having just spent cash to acquire/develop, the bot is
// often exactly that short. v6 adds a SWAP variant: pay the holdout with one of my
// junk lots (`junkLotForSwap` — a lone color lot in a set neither I nor the holdout
// have a stake in) plus minimal cash. Same block, fundable on a thinner bankroll
// (and cheaper when flush). Both variants are constructed; the selection takes
// whichever clears with the higher denial-augmented delta. `evaluateTrade` is still
// untouched, and the junk-lot filter plus the unchanged rival-threat pricing keep
// the in-kind sweetener from ever advancing the holdout toward a monopoly.
// ---------------------------------------------------------------------------
...
/** v6 — my cheapest "junk" lot I can hand `holder` to fund a denial in kind: a
 *  lone, building-free, un-mortgaged color property in a set I have no stake in,
 *  in a color `holder` has no stake in either — so parting with it neither
 *  sacrifices my own set progress nor advances the holdout toward a monopoly (the
 *  two ways an in-kind sweetener could backfire). Railroads/utilities are excluded
 *  (synergy). Lowest asset base first, so I give up the least value. Returns the
 *  lot position, or undefined if I have no such lot — in which case the swap is
 *  simply not offered and the cash buy stands. */
function junkLotForSwap(state: GameState, pid: string, holder: string): number | undefined {
  let pick: { pos: number; base: number } | null = null;
  for (const [posStr, owner] of Object.entries(state.ownership)) {
    if (owner !== pid) continue;
    const pos = Number(posStr);
    if (state.mortgaged[pos]) continue;
    const color = colorAt(pos);
    if (color === null) continue; // skip railroads/utilities — keep their synergy
    if (ownedInColor(state, pid, color) !== 1) continue; // lone lot — no set stake of mine
    if (ownedInColor(state, holder, color) !== 0) continue; // don't advance the holdout
    if (builtLotsInGroup(pos, (p) => developmentLevel(state, p)).length > 0) continue;
    const base = assetBase(state, pos);
    if (pick === null || base < pick.base) pick = { pos, base };
  }
  return pick === null ? undefined : pick.pos;
}
...
export function proposeBestTrade(
  ...
  // Offer C — TRADE-TO-DENY (v5), now fundable in kind (v6). For each rival one lot
  // short of a set whose last lot sits with a THIRD-PARTY holdout, buy that lot to
  // ME purely to block the completion. It doesn't complete my own set, so its worth
  // is the `DENY_FACTOR` premium on the rival set I'm preventing — the credit
  ...
  // its own denial.
  //
  // v6 adds a SECOND way to pay the holdout: a swap — hand them one of my junk lots
  // plus minimal cash, instead of cash alone. v5 could only deny when I could fund
  // the full sweetener in cash (`sweetenFor` returns null otherwise), so a denial I
  // wanted but couldn't afford simply didn't happen. Offering a junk lot in kind
  // makes the same block fire on a thinner bankroll (and a cash-rich one cheaper).
  // Both variants are pushed; the selection picks whichever clears with the higher
  // denial-augmented delta.
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      const positions = groupPositions(color);
      ...
      if (builtLotsInGroup(pos, (p) => developmentLevel(state, p)).length > 0) continue;
      const denyBonus = Math.round(monopolyBonus(color) * DENY_FACTOR);
      ...
      // v6 swap variant: fund the block in kind with a junk lot + minimal cash.
      const giveLot = junkLotForSwap(state, pid, holder);
      if (giveLot !== undefined) {
        const swap = sweetenFor(state, pid, holder, {
          propertyTo: { [pos]: pid, [giveLot]: holder },
          gojfTo: {},
          cashDelta: {},
        });
        if (swap !== null) {
          candidates.push({
            ...
            delta: 0,
            denyBonus,
            reason: `Swapping ${spaceName(giveLot)} to ${holderName} for ${spaceName(pos)} to deny ${opp.name} the ${colorName(color)} monopoly.`,
          });
        }
      }
    }
