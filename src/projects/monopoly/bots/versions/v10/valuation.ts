...
// opposite — the rival is a competing bidder who values its own completer at the
// FULL monopoly bonus, so a 0.6 ceiling drops out and lets the rival complete.
// Bidding up toward the full bonus either denies the set or forces the rival to
// overpay near its max — both negative-sum against that rival. `acquisitionValue`
// (buy/landing) is UNTOUCHED, so only the auction channel sharpens. Shared,
...
/** v10 AUCTION denial aggression. The 0.6 trade `DENY_FACTOR` is tuned for
 *  trade-to-deny, where the denied rival is NOT a party and can't bid — 0.6×bonus
 *  is enough to clear a holdout's sweetener. An AUCTION is the opposite: the rival
 *  is a competing bidder who values its own completer at the FULL monopoly bonus,
 *  so a 0.6 ceiling always drops out and the rival completes. v10 bids up toward
 *  the full bonus (factor 1.0 — denying is worth ≈ the whole swing the rival would
 *  gain) ONLY for a rival's pinpointed completer in the auction channel. Capped by
 *  `auctionBidCap` (net worth) so it can never bid into an unsettleable position;
 *  weak sets still self-gate (a small bonus rarely beats the going bid). */
const AUCTION_DENY_FACTOR = 1.0;
...
export function acquisitionValue(
  ...
  return mine + denyPremium(state, pid, pos, DENY_FACTOR);
}

/** The premium for taking `pos` away from a rival who is one lot short of the set
 *  it completes — `factor`×the set's monopoly bonus, or 0 if no rival is poised on
 *  exactly this lot. `pos` must be unowned (an open lot — the only kind an
 *  auction/landing offers). Shared by `acquisitionValue` (buy/landing, at the 0.6
 *  trade `DENY_FACTOR`) and `auctionValue` (auction, at the more aggressive
 *  `AUCTION_DENY_FACTOR`) so the one-short detection has a single source of truth. */
function denyPremium(state: GameState, pid: string, pos: number, factor: number): number {
  const color = colorAt(pos);
  if (color === null || pos in state.ownership) return 0;
  const total = groupPositions(color).length;
  let deny = 0;
  for (const opp of activeOpponents(state, pid)) {
    // `pos` is unowned, so the only seat one lot short of this set needs exactly
    // this lot — taking it blocks their monopoly.
    if (ownedInColor(state, opp.id, color) === total - 1) {
      deny = Math.max(deny, Math.round(monopolyBonus(color) * factor));
    }
  }
  return deny;
}

/** What `pos` is worth to bid up to in an AUCTION: its acquisition value to me,
 *  but with the rival-completer denial premium scaled by the aggressive
 *  `AUCTION_DENY_FACTOR` instead of the 0.6 trade factor — because in an auction
 *  the rival is bidding for the same lot at its full bonus, so a timid denial
 *  ceiling never wins. Identical to `acquisitionValue` when `pos` doesn't block a
 *  rival (no denial premium either way). The caller still clamps to what I can pay. */
export function auctionValue(state: GameState, pid: string, pos: number): number {
  const mine = positionValue(withOwner(state, pos, pid), pid) - positionValue(state, pid);
  return mine + denyPremium(state, pid, pos, AUCTION_DENY_FACTOR);
}
