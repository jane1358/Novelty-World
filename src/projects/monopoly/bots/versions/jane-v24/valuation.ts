...
function desiredLevel(
  ...
  // Opponent-aware hotel timing: if the weakest opponent is distressed,
  // push to hotels with a smaller cushion — max rent applies the kill shot.
  const opps = activeOpponents(state, pid);
  let weakestOpp: Player | null = null;
  for (const opp of opps) {
    if (!weakestOpp || opp.cash < weakestOpp.cash) weakestOpp = opp;
  }
  const oppDistressed = weakestOpp !== null && sellerDistress(state, weakestOpp.id) > 0.3;
  const killShot = cash > liquidityFloor(state, pid) + HOTEL_CUSHION / 2;
  if (scarce && opponentsWantHouses(state, pid)) {
    return { level: 4, why: "holding at 4 to starve the house bank" };
  }
  if ((flush || (oppDistressed && killShot)) && !scarce) {
    return { level: 5, why: oppDistressed ? "hotels to finish off a weak opponent" : "hotels for max rent" };
  }
