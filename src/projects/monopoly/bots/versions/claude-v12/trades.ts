...
// ===========================================================================
// v12 SNAPSHOT — fork of the champion v5's `trades.ts` (see EVOLUTION.md). v5's
// trade-to-deny construction is carried VERBATIM; v12's one change is the FIRST
// use of the replay-safe RNG seam (`./mix.ts`) — a MIXED tie-break over which
// near-equal trade to propose.
//
// v12 HYPOTHESIS: "an unpredictable bot denies a modelling opponent a clean read,
// transferring win share." v5 is fully deterministic, so its proposal on any board
// is fixed — a pro can anticipate it. v5 already ranks trade candidates by their
// `effective` (denial-augmented) delta and breaks ties by a FIXED color order, so
// when two near-equal trades are available it always picks the same one. v12
// instead MIXES among the candidates within `MIX_TOLERANCE` of the best, drawing
// the choice from `state.rngState` (see `./mix.ts` for why this is replay-safe and
// needs no Bot-contract change). This is the cleanest isolation of the information
// hypothesis: the mix only ever ranges over trades the bot rates as ~indifferent,
// so any win-share change is PURE unpredictability, not a value regression. A
// single eligible candidate reduces exactly to v5 (no choice to make).
//
// Everything else is v5 verbatim: `evaluateTrade`, the inherited completion
// search, the trade-to-deny construction (Offer C), the counterparty model, and
// the go/no-go gates are untouched — the change is only WHICH of several
// equally-good proposals is surfaced.
//
// (v5's carried denial: when a rival is one lot short of a set whose completer
// sits with a THIRD-PARTY holdout, buy that lot to DENY — priced off `DENY_FACTOR`,
// the rival not a party so it can't veto. See the v5 row in EVOLUTION.md.)
// ---------------------------------------------------------------------------
...
/** v12: candidates whose `effective` (denial-augmented) delta is within this many
 *  dollars of the best are treated as ~indifferent and MIXED over (which one to
 *  propose is drawn from `state.rngState`). Small relative to real candidate
 *  deltas (hundreds), so the mix never trades down for unpredictability — it only
 *  ranges over genuine near-ties, isolating the information effect from value. */
const MIX_TOLERANCE = 50;
...
export function proposeBestTrade(
  ...
  // Collect every PROPOSABLE, accepted, non-re-pitch candidate with its effective
  // (denial-augmented) delta, in the deterministic order they were constructed.
  const eligible: { cand: Candidate; effective: number }[] = [];
  for (const cand of candidates) {
    ...
    eligible.push({ cand, effective });
  }
  if (eligible.length === 0) return null;

  // v12 — MIXED tie-break. v5 took the strict argmax (fixed-order ties); v12 mixes
  // over the candidates within MIX_TOLERANCE of the best effective delta, so two
  // ~equally-good trades aren't always resolved the same way (denying a modelling
  // opponent a read). The draw comes from `state.rngState` and the eligible set is
  // in a deterministic order, so the pick is stable across the pacer's re-consults
  // within one decision window and reduces to v5 when only one candidate clears.
  const bestEffective = Math.max(...eligible.map((e) => e.effective));
  const contenders = eligible.filter((e) => e.effective >= bestEffective - MIX_TOLERANCE);
  const chosen = mixPick(contenders, state.rngState, `trade:${pid}`).cand;
  return { terms: chosen.terms, reason: chosen.reason };
}
