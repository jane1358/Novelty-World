// ===========================================================================
// search-v1 BASE POLICY — the claude-v38 dispatcher, copied VERBATIM from
// claude-v38/policy.ts (only the exported name changed: `claudeBot` -> `baseBot`).
//
// This is the greedy 1-ply value-maximizer that the SEARCH layer (`policy.ts`)
// improves on via truncated rollouts. At every decision point search does NOT
// target, the search bot delegates straight to this. Keeping the base as its own
// untouched copy is what makes the rollout's "all seats play the base policy"
// guarantee literal — the rollout driver consults `baseBot` for every seat.
//
// (claude-v38 provenance: a fork of v17 whose only change is the first-principles
// monopolyBonus in `./valuation.ts`; the dispatcher + trade engine are verbatim.)
// ===========================================================================
...
export function baseBot(state: GameState, playerId: string): BotDecision | null {
