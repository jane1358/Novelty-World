...
import { recentTradeLots } from "./tradememory";
import {
  acquisitionValue,
  activeOpponents,
  colorName,
...
export function proposeBestTrade(
  ...
  // its own denial.
  //
  // jane-v4 STRUCTURAL ANTI-CHURN: skip denial of lots that recently changed
  // hands via trade. These are "hot" — the premium-collector rotation that
  // produces the hot-potato ring. Completion trades (Offer A/B) are unaffected.
  const hotLots = recentTradeLots(state);
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      const positions = groupPositions(color);
      ...
      // jane-v4: skip denial of recently-churned lots to break hot-potato rings.
      if (hotLots.has(pos)) continue;
