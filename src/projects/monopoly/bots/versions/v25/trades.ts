...
// ===========================================================================
// v25 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). Valuation (bar two
// new exported helpers) and the dispatcher are v17 VERBATIM. The change is ONE
// thing and DEFENSIVE only: `rivalThreatCost` now prices HANDING a rival their 4th
// railroad / 2nd utility, so the bot won't SELL a rival its rail-set completer for
// face value — the leak v17 had (rivalThreatCost looped COLOR sets only, blind to
// the railroad/utility synergy a sold completer hands over). The basis is
// `kindCompletionBonus` (the synergy analog of `monopolyBonus`): the 4th rail's
// RAIL_SYNERGY 3→4 jump (200) or the 2nd utility (UTIL_PAIR_BONUS 40), apportioned
// the same way as a color premium. `evaluateTrade`, completion search, Offer C, and
// the counterparty model are otherwise v17 verbatim — only the THREAT term grows.
//
// NOT INCLUDED — proactive rail denial (an Offer C that buys a rival's rail
// completer from a holdout). It was prototyped and REJECTED in design: railroads
// reproduce Finding 1's phantom-denial HOT-POTATO. A rail split is STATIC (never
// develops/busts to resolve) and the rival's completion gain CLEARS the
// threat-priced extraction cost (unlike brown, which v14's gate catches), so two
// non-rival bots pass the completer back and forth forever, each booking the denial
// credit for zero progress. The root cause is that a trade-denial never TRULY
// blocks in a bot field — the new holder would re-sell at threat-price — so the
// proactive lever degenerates into churn here. Offer C therefore stays color-only;
// this version keeps just the safe, churn-free DEFENSIVE half.
// ---------------------------------------------------------------------------
...
function rivalThreatCost(
  ...
  for (const opp of activeOpponents(state, pid)) {
    ...
    // v25: handing a rival their 4th railroad / 2nd utility completes a synergy
    // set just as a color monopoly does — price it the same apportioned way, so
    // the bot won't sell a rival its rail-set completer for face value.
    for (const kind of ["railroad", "utility"] as const) {
      const total = kind === "railroad" ? 4 : 2;
      if (ownedOfKind(after, opp.id, kind) !== total) continue;
      if (ownedOfKind(state, opp.id, kind) === total) continue; // already complete — no new threat
      let received = 0;
      let fromPid = 0;
      for (const [posStr, to] of Object.entries(terms.propertyTo)) {
        const pos = Number(posStr);
        if (to !== opp.id || SPACES[pos].kind !== kind) continue;
        received += 1;
        if (state.ownership[pos] === pid) fromPid += 1;
      }
      if (received === 0) continue; // rival completed it without my lots — not my threat
      share += kindCompletionBonus(kind) * RIVAL_THREAT_FACTOR * (fromPid / received);
    }
...
export function proposeBestTrade(
  ...
  // Offer C — TRADE-TO-DENY (v5). For each rival one lot short of a COLOR set whose
  ...
  // `DENY_FACTOR` premium on the rival set I'm preventing. The rival is NOT a party
  // (only the holdout and I are), so it can't veto its own denial.
  //
  // v25 leaves this COLOR-ONLY on purpose: extending it to railroads reproduces
  // Finding 1's phantom-denial hot-potato (rail splits are static and clear v14's
  // acquirability gate, so the completer bounces bot→bot forever). See the header.
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      const positions = groupPositions(color);
