// ===========================================================================
// v17 SNAPSHOT — fork of v14 (the phantom-denial-fixed base; see EVOLUTION.md).
...
/** v25 — the position-value a player gains by COMPLETING the non-color synergy
 *  "sets": the railroad set (the 3→4 jump in `RAIL_SYNERGY`) and the utility pair
 *  (`UTIL_PAIR_BONUS`). The rail/utility analog of a color's `monopolyBonus`, so
 *  the trade engine can price denying — or handing over — these sets the same way
 *  it prices color monopolies. Railroads accrue synergy incrementally (owning 3
 *  already pays), so only the marginal completion gain is denial-relevant, mirror
 *  of how a color's N-1 lots carry no set value until the last one lands. */
export function kindCompletionBonus(kind: "railroad" | "utility"): number {
  return kind === "railroad" ? RAIL_SYNERGY[4] - RAIL_SYNERGY[3] : UTIL_PAIR_BONUS;
}
