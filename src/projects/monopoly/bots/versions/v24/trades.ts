...
// ===========================================================================
// v24 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). The valuation,
// dispatcher, and the rest of the trade engine are v17 VERBATIM; the ONE change
// is in `proposeBestTrade`: the bot will now ASSEMBLE A MONOPOLY FROM SCRATCH —
// buy EVERY lot of a high-value color it currently holds NONE of, off willing
// opponents, in one N-party deal.
//
// THE GAP: v17's construction only completes a set the bot already holds a STAKE
// in (`if (owned === 0 ...) continue` — "buying a whole color from scratch isn't
// this engine's job"). That self-imposed limit leaves a +EV move on the table:
// when a prize set is SPLIT among opponents (none holding the monopoly), the bot
// can buy all its lots and seize the set. Each seller prices the rival-monopoly
// it hands over (`RIVAL_THREAT_FACTOR` 0.6×bonus, apportioned by
// `rivalThreatCost`), so the bot pays ~deeds + 0.6×bonus and GAINS bonus + deeds
// — a net ≈ 0.4×bonus by its own `positionValue` (orange ≈ +164). It is the
// proven AGGRESSION shape (v17) and an ASYMMETRIC grab (the set comes off the
// board for everyone else), exploiting opponents willing to sell property for
// cash.
//
// TWO GUARDRAILS keep it the WINNING kind of aggression, not the losing kind:
// (1) `ACQUIRE_MIN_GAIN` — the grab must net a real PRIZE, far above the
// break-even noise `ACCEPT_MIN` allows, so the bot doesn't sink ~$900 of capital
// into a cheap set worth +$40 (this self-excludes pink/light-blue/brown, whose
// thin 0.4×bonus can't clear the bar); (2) the bot must stay above its rent
// reserve after paying, so it never buys a BARE monopoly it then can't develop —
// the dead-set trap. Completion/denial candidates keep v17's plain gates; only
// the from-scratch grab is gated. Buying an INTACT monopoly off one owner is
// already -EV (they lose the bonus and charge for it) and self-rejects.
// ---------------------------------------------------------------------------
// v14 SNAPSHOT — fork of the champion v5's `trades.ts` (see EVOLUTION.md). v5's
...
/** v24 — minimum net gain (by `positionValue`) for a FROM-SCRATCH monopoly grab
 *  to be worth its capital. Far above `ACCEPT_MIN`: assembling a set sinks real
 *  cash (~deeds + the sellers' 0.6×bonus premium), so a marginal +$40 cheap set
 *  isn't worth the liquidity hit. Calibrated against the ≈0.4×bonus a split-set
 *  grab nets — clears the prize sets (orange ≈ +164, red ≈ +158, yellow/green/
 *  dark-blue ≈ +100–130) and self-excludes pink/light-blue/brown (whose thin
 *  bonus can't clear the bar). A first-cut dial; the A/B will say if it's right. */
...
interface Candidate {
  ...
  denyBonus?: number;
  /** v24 — true for a from-scratch monopoly grab (a color I hold NONE of). It
   *  passes v17's standard accept gate but ALSO the stricter `worthAcquiring`
   *  prize/liquidity gate, since it starts a set rather than finishing one. */
  fromScratch?: boolean;
}

/** v24 — is a from-scratch monopoly grab worth its capital? Two tests: the gain
 *  must clear `ACQUIRE_MIN_GAIN` (a real prize, not break-even noise), and the
 *  bot must stay at/above its rent reserve after paying, so it never buys a BARE
 *  set it can't then develop (the dead-monopoly trap). Completions/denials —
 ...
 *  which closes the 1-1-1 / two-short boards no 2-way deal can; (v24) per
 *  high-value color I hold NONE of, a from-scratch GRAB of the whole set off its
 *  opponent owners (gated on being a real prize that keeps me above my reserve);
 *  and, per rival one
...
export function proposeBestTrade(
  ...
  for (const color of COLORS_BY_WEIGHT) {
    ...
    if (owned === positions.length) continue; // already own the whole set
    // v24: `owned >= 1` is a COMPLETION (finish a near-monopoly — v17 behavior);
    // `owned === 0` is a from-scratch ACQUISITION (buy the whole color off
    // willing opponents to seize the monopoly outright). The grab is gated
    // harder in the selection pass (`worthAcquiring`: real prize + stays above
    // the rent reserve), since it sinks capital to START a set, not finish one.
    const fromScratch = owned === 0;
    ...
    if (buy !== null) {
      const fromWhom =
        sellers.size === 1 ? "its owner" : `${sellers.size.toString()} owners`;
      const reason = fromScratch
        ? `Buying the whole ${colorName(color)} set from ${fromWhom} to seize the monopoly outright.`
        : single !== undefined
          ...
          : `Buying the ${missingLots.length.toString()} missing ${colorName(color)} ` +
            `from ${fromWhom} to complete the monopoly.`;
      candidates.push({ terms: buy, delta: 0, reason, fromScratch });
    }
  }
  ...
  for (const cand of candidates) {
    ...
    // v24: a from-scratch monopoly grab must also be a real prize and leave the
    // bot above its rent reserve — it starts a set rather than finishing one.
    if (cand.fromScratch && !worthAcquiring(state, pid, cand.terms, mine.delta)) continue;
