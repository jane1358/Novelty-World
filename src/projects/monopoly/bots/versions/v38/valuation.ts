// ===========================================================================
// v38 SNAPSHOT — fork of the champion v29 (see EVOLUTION.md).
// v38 adds SPREAD DEVELOPMENT: planBuild now uses two passes — first get ALL
// monopolies to 3 houses (the biggest rent-per-dollar jump), THEN push the best
// set higher. The current code develops greedily one-set-to-max before the next
// gets anything. Distress pricing verbatim from v29. — fork of the champion v28 (see EVOLUTION.md). IDENTICAL to v28
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // v38: Helper to try a specific level on a color set, given the accumulated
  //  build so far. Returns true if the commit was affordable.
  function tryLevel(
    color: PropertyColor,
  ...
  // v38 PASS 1: Get ALL monopolies to at least 3 houses (the biggest
  //  rent-per-dollar jump). This maximizes the number of developed sets
  //  generating rent, covering more of the board.
  ...
  for (const m of myMonopolies) {
    ...
    // Try from target down to current level
  ...
  // v38 PASS 2: Push the best sets higher (toward hotels) now that everything
  //  has at least 3. Walk best-first again, pushing each as high as affordable.
  if (want > SPREAD_FLOOR) {
    for (const m of myMonopolies) {
      if (m.locked) continue;
      const floorOf = maxLevelInSet(state, m.color);
      // Re-read floorOf from the accumulated build
