// ===========================================================================
// v23 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). Carries v17's lower
// liquidity reserve verbatim and adds ONE change, in `planBuild`: reclaim a dead
// (mortgaged) monopoly MORE EAGERLY.
//
// THE LEAD: a mortgaged monopoly earns NOTHING — it can't be built and its rent is
// frozen — so reactivating it (unmortgaging to restore the set's double-rent and
// unfreeze it for building) is an unusually high-value-per-dollar redeploy of idle
// capital. v17's planBuild only does this when comfortably "flush" (cash > floor +
// `HOTEL_CUSHION` 600), so a set can sit dead for many turns while the bot is merely
// solvent. v23 reactivates it at a thinner cushion (floor + `RECLAIM_CUSHION` 200),
// deploying idle capital sooner — the "deploy capital faster" direction v17 proved
// wins, but on a DISTINCT gate (the unmortgage-reclaim) and as a step-change in value
// (a frozen set turning back on), not the marginal buy/build/reserve nudges that
// washed (v18/v20/v21). Isolated to planBuild's reclaim gate; everything else (the
// reserve, desiredLevel, the whole trade engine) is v17 verbatim.
// ===========================================================================
...
/** v23 — spare cash (above the floor) at which the bot will RECLAIM a dead (mortgaged)
 *  monopoly: unmortgage it to restore double-rent and unfreeze it for building. Lower
 *  than `HOTEL_CUSHION` on purpose — reactivating a set that earns NOTHING while
 *  mortgaged is a higher-value-per-dollar redeploy than pushing a live set to hotels, so
 *  it's worth doing at a thinner cushion (v17 waited for full "flush", leaving idle
 *  capital frozen for many turns). */
const RECLAIM_CUSHION = 200;
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // v23: reclaim a dead (mortgaged) monopoly at a thinner cushion than full "flush" —
  // reactivating a set that earns nothing is a high-value redeploy worth doing sooner.
  const canReclaim = player.cash > floor + RECLAIM_CUSHION;
  ...
  for (const color of COLORS_BY_WEIGHT) {
    ...
    if (locked && !canReclaim) continue; // don't reclaim a mortgaged set while thin
