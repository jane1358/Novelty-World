// ===========================================================================
// v36 SNAPSHOT — fork of the champion v29 (see EVOLUTION.md).
// v36 adds ACCELERATED CLOSEOUT: when the bot is ahead in monopolies, lower
// the liquidity reserve to deploy cash into development faster — converting
// a property lead into a rent lead that bankrupts opponents before they catch
// up. Distress pricing carried verbatim from v29. — fork of the champion v28 (see EVOLUTION.md). IDENTICAL to v28
...
/** How many complete color monopolies does `pid` own? */
function monopolyCount(state: GameState, pid: string): number {
  let count = 0;
  for (const color of COLORS_BY_WEIGHT) {
    if (hasMonopoly(state, color, pid)) count++;
  }
  return count;
}

/** Is `pid` ahead on the board — more monopolies than any single opponent?
 *  Returns the margin (positive = ahead by N, negative = behind). */
function monopolyMargin(state: GameState, pid: string): number {
  const mine = monopolyCount(state, pid);
  let bestOpp = 0;
  for (const opp of activeOpponents(state, pid)) {
    bestOpp = Math.max(bestOpp, monopolyCount(state, opp.id));
  }
  return mine - bestOpp;
}

/** v36: When ahead on monopolies, deploy capital faster. The floor scales down
 *  proportionally to the lead — each monopoly of margin shaves 15% off the
 *  reserve (capped at 60%), because the rent income from being ahead covers
 *  the risk of holding less cash. */
const LEAD_FLOOR_DISCOUNT = 0.15; // per monopoly of margin
const LEAD_FLOOR_CAP = 0.60;      // max 60% reduction
...
export function liquidityFloor(state: GameState, pid: string): number {
  ...
  // v36: accelerated closeout — lower the floor when ahead on monopolies
  const margin = monopolyMargin(state, pid);
  if (margin > 0) {
    const discount = Math.min(LEAD_FLOOR_CAP, LEAD_FLOOR_DISCOUNT * margin);
