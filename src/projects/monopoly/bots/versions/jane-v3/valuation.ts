// ===========================================================================
// jane-v3 SNAPSHOT — fork of jane-v2 (see EVOLUTION.md).
// jane-v3 makes the SPREAD_FLOOR dynamic: 4 houses when flush (more board
// coverage from higher rents), 2 when cash-tight (cheaper to spread, still
// gets the monopoly double-rent). jane-v2 used a static floor of 3. — fork of jane-v1 (see EVOLUTION.md).
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // PASS 1: Get ALL monopolies to a baseline level first.
  // jane-v3: dynamic floor — 4 when flush (maximize board coverage with higher
  //  rents), 2 when cash-tight (cheaper spread, still unlocks monopoly rent).
  const SPREAD_FLOOR = flush ? 4 : 2;
