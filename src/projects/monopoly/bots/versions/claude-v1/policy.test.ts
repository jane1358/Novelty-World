...
// freshGame seats p1..p4, p1 active, $1500 each, no ownership. policy reads
...
describe("policy — buy / raise-to-buy", () => {
  ...
  it("passes on a low-value buy that would breach its rent reserve", () => {
    ...
    expect(policy(state, "p1")?.intent).toEqual({
      kind: "decline-buy",
      playerId: "p1",
    });
  });
  ...
  it("stages the mortgage raise, then commits the buy, in raising-cash", () => {
    ...
    expect(policy(staging, "p1")?.intent).toEqual({
    ...
    expect(policy(ready, "p1")?.intent).toEqual({ kind: "buy", playerId: "p1" });
  });
});

describe("policy — auction", () => {
...
describe("policy — must-raise-cash (value-preserving)", () => {
...
describe("policy — jail as a haven", () => {
  ...
  it("notes why it stays in jail, then rolls (null) once the note is logged", () => {
    ...
    expect(policy(noted, "p1")).toBeNull();
  });
...
describe("policy — trade evaluation", () => {
...
describe("policy — proactive arming at pre-roll", () => {
  ...
  it("does nothing at pre-roll with no monopoly to build and no trade to make", () => {
    expect(policy(withTurn({ phase: "pre-roll" }), "p1")).toBeNull();
  });
});

describe("policy — unmortgage and redeploy", () => {
  ...
  it("does NOT unmortgage when it isn't comfortably above its rent reserve", () => {
    ...
    expect(policy(preRoll, "p1")).toBeNull();
    ...
    expect(policy(managing, "p1")).toBeNull();
  });
});

describe("policy — trade-building drive", () => {
  it("populates an empty draft, then proposes once it matches the plan", () => {
    ...
    expect(policy(ready, "p1")?.intent).toEqual({ kind: "propose-trade", playerId: "p1" });
  });
});
