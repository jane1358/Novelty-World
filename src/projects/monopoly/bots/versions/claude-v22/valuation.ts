// ===========================================================================
// v22 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). Carries v17's lower
// liquidity reserve verbatim and adds ONE change, here in `desiredLevel`: a HOUSE-FAMINE
// DENIAL lever — the proven negative-sum shape (v5) on a new channel, the 32-house bank.
//
// THE LEAD: holding houses out of the bank starves rivals of development (a real pro
// tactic). v17 already holds at 4-and-hold rather than hoteling — which would free 4
// houses back to the bank — but ONLY once the bank is nearly empty (≤ `HOUSE_SCARCE` 6).
// By then the famine is largely decided. v22 starts the famine-hold while the bank is
// merely DRAWING DOWN (≤ `HOUSE_RACE` 12) and a rival could use houses: it caps its own
// development at 4 (foregoing the hotel rent bump) to keep houses locked away and win the
// race to starve rivals' monopolies. This SACRIFICES my own max rent for the denial — a
// negative-sum move whose whole value is setting back specific rivals, the v5 shape, on
// the development channel (where v19's positive-sum rent maximization regressed). Off the
// exhausted liquid-deployment axis (it deploys LESS, not more). Isolated to
// `desiredLevel`; the reserve, planBuild, and the whole trade engine are v17 verbatim.
// ===========================================================================
...
/** v22 — the FAMINE RACE threshold. While the 32-house bank is merely drawing down
 *  (≤ this many left) and a rival could use houses, the bot already starts holding at
 *  4-and-hold rather than hoteling, to keep houses locked out of the bank and win the
 *  race to starve rivals' development — well before the hard `HOUSE_SCARCE` floor where
 *  the famine is already decided. Sacrifices my own hotel rent for the denial. */
const HOUSE_RACE = 12;
 ...
 *  - (v22) the house bank is drawing down (≤ HOUSE_RACE) AND a rival could use houses
 *    → 4 and HOLD: keep houses locked out of the bank to win the famine race and starve
 *    rivals' development, foregoing the hotel rent bump (going to a hotel would hand four
 *    houses back). Subsumes v17's hard-scarce hold (≤ HOUSE_SCARCE is a stricter case);
 *  - flush AND houses aren't scarce AND no famine race on → hotel for maximum rent;
...
function desiredLevel(
  ...
  const housesLeft = bankSupply(state).houses;
  const scarce = housesLeft <= HOUSE_SCARCE;
  const flush = cash > liquidityFloor(state, pid) + HOTEL_CUSHION;
  if (housesLeft <= HOUSE_RACE && opponentsWantHouses(state, pid)) {
    return {
      level: 4,
      why: scarce
        ? "holding at 4 to starve the house bank"
        : "holding at 4 to win the house famine while the bank draws down",
    };
  }
  if (flush && !scarce) {
