...
// ===========================================================================
// v34 SNAPSHOT — fork of v28 (the champion; see EVOLUTION.md).
// v34 adds MORTGAGE-TO-FUND for trade sweeteners: when cash can't cover the
// sweetening, mortgage idle non-monopoly lots to fund it. This is explicitly
// untested (v4 tested mortgage-for-development only, not for trade sweeteners). — fork of v17 (the champion; see EVOLUTION.md).
...
function sweetenFor(
  ...
  if (myCash - cash < 0) {
    // v34: mortgage idle non-monopoly lots to fund the sweetener. The
    //  monopoly bonus from completing the set is worth the 10% mortgage
    //  interest on a non-essential property.
    const mortgage = planRaiseByMortgage(state, pid, cash - myCash);
 ...
 *  fund the total in cash without going negative (v3, like v2, has v34 adds
 *  the mortgage-to-fund path for trade sweeteners). */
function sweetenForAll(
  ...
  const myCash = state.players.find((p) => p.id === pid)?.cash ?? 0;
  if (myCash - total < 0) {
    // v34: mortgage idle non-monopoly lots to fund the combined sweetener.
    const mortgage = planRaiseByMortgage(state, pid, total - myCash);
    if (mortgage === null) return null;
