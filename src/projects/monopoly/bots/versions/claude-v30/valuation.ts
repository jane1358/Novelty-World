// ===========================================================================
// v30 SNAPSHOT — fork of the champion v29 (see EVOLUTION.md). IDENTICAL to v29
// except the distress GATE: v30 adds `DISTRESS_MARGIN` 1.3, widening `isDistressed`
// from v29's strict "can't cover one deadly hit at all" to "can't comfortably cover
// it" (left broke after one hit also qualifies), so the proven desperation discount
// fires on MORE genuinely-cornered seats. The discount magnitude (1.0), the seller
// scale wiring, the buyer construction, and v17's reserve are all v29 verbatim.
// ===========================================================================
...
/** v30 — how far ABOVE the single deadliest rent a seat's total liquidity can
 *  reach and still count as distressed. v29's gate fired only when one deadly hit
 *  busts the seat outright (`liquidity < deadlyRent`, margin 1.0). But a seat that
 *  can *barely* cover one hit is still cornered — paying it leaves it broke for the
 *  next landing, and developed boards rarely sit alone. v30 widens the trigger to
 *  `liquidity < deadlyRent × DISTRESS_MARGIN`, so "one hit from BROKE" qualifies,
 *  not only "one hit from BANKRUPT". Kept modest (1.3) so it still means real
 *  danger — NOT "low-ish cash", the failure mode the lead warns against. */
const DISTRESS_MARGIN = 1.3;
...
/** Is `pid` distressed — one bad landing from broke? True only when a DEADLY
 *  developed rent is out there AND `pid`'s total liquidity (`cash +
 *  mortgageableTotal`) can't comfortably cover it — v30 widens v29's strict
 *  "can't cover it at all" to "can't cover it × `DISTRESS_MARGIN`", so a seat that
 *  would be left broke after one hit also qualifies. Still real danger, not
 *  "low-ish cash": the developed-rent floor (`DEADLY_RENT`) keeps it from firing on
 *  a quiet board, and the margin is modest. */
export function isDistressed(state: GameState, pid: string): boolean {
  ...
  return player.cash + mortgageableTotal(state, pid) < deadly * DISTRESS_MARGIN;
}
