...
import { isDistressed as v29IsDistressed } from "../claude-v29/valuation";
...
describe("v28 distress model — isDistressed", () => {
  ...
  it("v30 widens the gate — flags a 'broke after one hit' seat v29 would not", () => {
    // The widened gate (margin 1.3) is a strict SUPERSET of v29's (margin 1.0):
    // anything v29 flags, v30 flags; and there's a cash band — covers one deadly
    // hit but is left broke — that ONLY v30 flags. Scan to prove both.
    let foundWiderHit = false;
    for (let cash = 0; cash <= 4000; cash += 100) {
      const s = setCash(withDeadlyBoard({ 19: "p2" }), "p2", cash);
      const v29Flag = v29IsDistressed(s, "p2");
      const v30Flag = isDistressed(s, "p2");
      // v30 is a superset: never flags less than v29.
      expect(!v29Flag || v30Flag).toBe(true);
      if (!v29Flag && v30Flag) foundWiderHit = true;
    }
    expect(foundWiderHit).toBe(true); // the widened band is non-empty
  });
});
