...
export function proposeBestTrade(
  ...
  return pickBest(state, pid, candidates);
}

/** v32 — MORTGAGE-TO-FUND a DISTRESS COMPLETION (roadmap #2's open sweetener half,
 *  HARD-GATED to the proven distress shape). Detect a completion that is +EV and
 *  acceptable to every seller and would fire EXCEPT that I can't fund the cash
 *  sweetener outright — but COULD by mortgaging idle lots. Returns the cash
 *  shortfall `need` (and the color, for the note); null when no such completion
 *  exists. The hard gate, in order:
 *
 *  - at least one seller of a missing lot is GENUINELY distressed (`isDistressed`)
 *    — this is the fleeting, asymmetric, UNDERPRICED opportunity v28/v29 win on,
 *    NOT a general mortgage-to-fund (which would be v4's washed plain tempo);
 *  - the completion is +EV for me after the priced rival threat (`evaluateTrade`
 *    on the fully-sweetened terms) and every seller accepts it — a deal that would
 *    actually clear if funded;
 *  - it is blocked SOLELY by cash: the sweetener `total` exceeds my cash but is
 *    within reach of `cash + mortgageableTotal`. (If cash already covers it, Offer
 *    B proposes it directly — no mortgage needed. If even mortgaging out can't
 *    reach it, the completion is genuinely unaffordable — don't bleed interest on a
 *    raise that still leaves the trade unfundable.)
 *
 *  This is the ONE genuinely-open corner the Batch-3 audit surfaced: v29 returns
 *  null here because `sweetenForAll` bails when `myCash < total`. The mechanism is
 *  CROSS-TURN (no in-trade raise phase exists): the policy arms a mortgage-only
 *  `manage` now, then proposes the completion next turn once cash covers it. */
export function distressCompletionNeedingCash(
  state: GameState,
  pid: string,
): { need: number; color: string } | null {
  const myCash = state.players.find((p) => p.id === pid)?.cash ?? 0;
  const headroom = mortgageableTotal(state, pid);
  ...
  for (const color of COLORS_BY_WEIGHT) {
    ...
    const sellers = [...new Set(missingLots.map((m) => m.owner))];
    // GATE 1: a genuinely distressed seller — the fleeting underpriced opportunity.
    if (!sellers.some((sId) => isDistressed(state, sId))) continue;

    const propertyTo: Record<number, string> = {};
    for (const m of missingLots) propertyTo[m.pos] = pid;
    const base: TradeTerms = { propertyTo, gojfTo: {}, cashDelta: {} };

    // The cash sweetener total to clear every seller's (distress-discounted)
    // break-even — the same per-seller minima `sweetenForAll` composes.
    let total = 0;
    for (const sId of sellers) {
      const delta = evaluateTrade(state, sId, base).delta;
      if (delta >= ACCEPT_MARGIN) continue;
      total += Math.ceil(ACCEPT_MARGIN - delta);
    }
    // GATE 3: blocked SOLELY by cash (cash short, but within mortgage reach).
    if (total <= myCash) continue; // Offer B already proposes it — no mortgage needed
    if (total > myCash + headroom) continue; // unaffordable even mortgaged out

    // GATE 2: the funded completion must be +EV for me and acceptable to every
    // seller — exactly what Offer B's go/no-go would check once funded.
    const funded: TradeTerms = {
      ...base,
      cashDelta: { ...sellerSweeteners(state, sellers, base), [pid]: -total },
    };
    const mine = evaluateTrade(state, pid, funded);
    if (!mine.accept) continue;
    if (!sellers.every((sId) => evaluateTrade(state, sId, funded).accept)) continue;

    return { need: total - myCash, color: colorName(color) };
  }
  return null;
}

/** Per-seller cash sweeteners for `base` (the distress-discounted break-even +
 *  margin), shared by `distressCompletionNeedingCash`'s funded-terms check. */
function sellerSweeteners(
  state: GameState,
  sellers: readonly string[],
  base: TradeTerms,
  ...
  for (const sId of sellers) {
    const delta = evaluateTrade(state, sId, base).delta;
    if (delta >= ACCEPT_MARGIN) continue;
    out[sId] = Math.ceil(ACCEPT_MARGIN - delta);
  }
  return out;
}

/** Rank the constructed candidates and return the best proposable one. */
function pickBest(
  state: GameState,
  pid: string,
