// ===========================================================================
// opt-v1 SNAPSHOT — claude-v38's policy with an ES-OPTIMIZED parameter vector.
//
// This file is a SELF-CONTAINED copy of the proven parameterized factory
// (`bots/optimize/bot.ts` in the search-paradigm worktree): claude-v38's
// valuation.ts + trades.ts + policy.ts with the 15 tuning constants read from a
// `ParamVector` instead of hardcoded module `const`s. Every piece of LOGIC is
// claude-v38 VERBATIM; the only change is that the tuned constants come from `p`.
//
// FIDELITY BY CONSTRUCTION: with `DEFAULT_PARAMS` (claude-v38 verbatim) this
// factory is byte-identical to claude-v38 — pinned in the optimizer worktree by
// `optimize/param-fidelity.test.ts` and re-pinned here in `policy.test.ts`. opt-v1
// is built by binding this SAME proven factory to the ES-winning vector
// (`OPT_V1_PARAMS` in `index.ts`), NOT by hand-transcribing 15 numbers into a copy
// of claude-v38 — so the frozen bot is provably the bot the ES optimized.
//
// This snapshot is intentionally STANDALONE: it imports only shared monopoly
// infrastructure (`../../../data`, `../../../logic`, …) and the local `Bot`
// contract. No cross-version imports, no `optimize/` imports, no `Math.random` /
// `Date` — all in-game randomness flows through `state.rngState` in the engine, so
// the bound bot is a pure, deterministic function of state (replay intact).
// ===========================================================================
