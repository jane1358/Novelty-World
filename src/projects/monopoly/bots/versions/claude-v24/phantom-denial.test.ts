...
describe("v24 — phantom-denial gate (inherited from v14, Finding 1)", () => {
  it("does NOT build the brown hot-potato denial that v5 builds", () => {
    ...
    expect(v24Propose(ring, "p1")).toBeNull(); // v14: no phantom buy
  });
  ...
  it("STILL builds a real denial of a strong set (orange) — identical to v5", () => {
    ...
    const deal = v24Propose(strong, "p1");
  ...
  it("does NOT deny a strong set the rival is too cash-poor to acquire", () => {
    ...
    // denial is phantom. v5 still buys to deny; v24 gates that phantom denial. (v24
    // does instead GRAB the split orange set for itself — a separate, real +EV move
    // pinned in `acquire.test.ts` — so the invariant here is "no phantom DENIAL",
    // not "no proposal".)
    ...
    expect(v24Propose(poorRival, "p1")?.reason ?? "").not.toContain("deny");
  });
  ...
  it("leaves completion construction untouched (matches v5)", () => {
    ...
    expect(v24Propose(completion, "p1")).toEqual(v5Propose(completion, "p1"));
  });
});
