...
import { liquidityFloor as v27Floor } from "./valuation";

// v27 inherits v17's lower voluntary-spend liquidity reserve VERBATIM (v27's only
...
describe("v27 liquidity floor — inherits v17's lower reserve", () => {
  it("reserves strictly less on a developed board (more cash for offense)", () => {
    const state = withHotel();
    const v14 = v14Floor(state, "p1");
    const v27 = v27Floor(state, "p1");
    expect(v27).toBeLessThan(v14);
    expect(v27).toBeLessThanOrEqual(300); // v17 cap, carried into v27
  ...
  it("matches v14 on a quiet board (both clamp to the shared BASE_FLOOR)", () => {
    ...
    expect(v27Floor(quiet, "p1")).toBe(v14Floor(quiet, "p1"));
  });
});
