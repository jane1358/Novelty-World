...
// ===========================================================================
// v34 SNAPSHOT — branched from champion v29. The ONLY change is the v34 ANTI-CHURN
// COOLDOWN on Offer C below; the desperation-acquisition mechanism, valuation, and
// dispatcher are v29 VERBATIM. Exposed as `claudeV34Bot`.
//
// THE v34 FIX (strong-set hot-potato, take 2): v33 tried to PRICE the denial away
// (skip if the rival could re-acquire the completer from me) and REGRESSED — 47.5%
// vs v29 — because that gate keyed on the rival's wealth and so deleted the whole
// denial mechanism (first denials AND re-hops), and the proactive strong-set denial
// carries real win share (~15 Elo). The lesson: the win value is in the denial; the
// pathology is only the REPETITION. v34 attacks just the repetition — a per-lot
// cooldown (`tradedWithin` + the gate in Offer C). The first bot to deny a completer
// buys it and HOLDS it; because the lot was just traded, every other bot now skips
// re-buying it for DENY_COOLDOWN_TURNS, so it sits still (the rival stays blocked)
// instead of rotating bot→bot. Removes only no-progress re-hops — which by
// construction don't change who is denied — so it should be win-NEUTRAL where v33
// was win-NEGATIVE. (Validated by the K-sweep triage vs v29; see EVOLUTION.md.)
//
// v29's inherited mechanism (unchanged) — VERBATIM v28's trades.ts (v29's only
// change is the `DISTRESS_DISCOUNT` constant in valuation.ts). The mechanism below
// is v28's: v28 added the DESPERATION-ACQUISITION lead (lead b) as ONE coupled hypothesis
...
/** v34 ANTI-CHURN COOLDOWN (turns): once a completer has changed hands by trade,
 *  the rival it would complete a set for is already blocked by whoever now holds it
 *  — re-buying it to "deny" the same rival again is the hot-potato (the observed
 *  dark-blue ring), pure no-progress shuffling. So a lot that was traded within this
 *  many turns is OFF-LIMITS for a fresh DENIAL buy (Offer C only): let it settle.
 *  The first denial still fires (nothing traded it yet); the re-hops don't. Tuned
 *  by the K-sweep — small enough to allow a genuine re-denial once the board has
 *  truly moved on, large enough to span a ring (which re-hops every turn). */
const DENY_COOLDOWN_TURNS = 8;
...
/** v34 — has `pos` changed hands by TRADE within the last `withinTurns` turns? A
 *  completer that was just traded is already parked with a non-rival who blocks the
 *  rival; re-buying it to deny the SAME rival again is the no-progress hot-potato.
 *  Turn numbers are unique and ascending, so we scan back from the current turn and
 *  stop once outside the window. Pure read of the event log (in `state.turns`), so
 *  replay is unaffected. */
function tradedWithin(state: GameState, pos: number, withinTurns: number): boolean {
  const groups = state.turns;
  if (groups.length === 0) return false;
  const current = groups[groups.length - 1].turn;
  for (let i = groups.length - 1; i >= 0; i--) {
    const g = groups[i];
    if (current - g.turn >= withinTurns) break;
    for (const e of g.events) {
      if (e.kind === "trade" && (Object.hasOwn(e.propertyTo, pos) || Object.hasOwn(e.propertyFrom, pos))) {
        return true;
      }
    }
  }
...
export function proposeBestTrade(
  ...
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      ...
      // v34 ANTI-CHURN COOLDOWN: if this completer was traded within the last
      // DENY_COOLDOWN_TURNS, it is already parked with a non-rival who blocks the
      // rival and is just being shuffled — re-denying it is the hot-potato. Let it
      // settle. The FIRST denial still fires (nothing has traded it yet); only the
      // no-progress re-hops are suppressed, so the win-carrying denial is intact
      // while the visible ring is broken.
      if (tradedWithin(state, pos, DENY_COOLDOWN_TURNS)) continue;
