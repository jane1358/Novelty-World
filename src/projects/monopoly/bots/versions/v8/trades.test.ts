...
import { proposeBestTrade as v8Propose } from "./trades";

// v8 carries v5's TRADE-TO-DENY engine verbatim (its own change is the v4-tempo
// coupling in valuation.ts). These tests pin the denial behavior is intact in v8:
// block a rival one lot short of a set by buying the completer from a third-party
// holdout, even though it doesn't complete my own set. v3 only constructs deals
// that complete MY sets, so on a pure-denial board it proposes nothing; v8 surfaces
// the block. Oranges = {16, 18, 19}; pinks = {11, 13, 14}; freshGame seats p1..p4.
...
describe("v8 proposeBestTrade — trade-to-deny (v5 engine, carried)", () => {
  it("buys a rival's completer from a holdout to deny, where v3 proposes nothing", () => {
    ...
    const proposal = v8Propose(state, "p1");
  ...
  it("does NOT deny when the completer is unowned (that's a buy/auction, not a trade)", () => {
    ...
    expect(v8Propose(state, "p1")).toBeNull();
  });
  ...
  it("adds no denial when I already hold the rival's completer (matches v3)", () => {
    ...
    expect(v8Propose(state, "p1")).toEqual(v3Propose(state, "p1"));
  });
