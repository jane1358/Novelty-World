...
// ===========================================================================
// v33 SNAPSHOT — branched from champion v29. The ONLY change is the v33
// MARGINAL-DENIAL GATE in Offer C below (the strong-set hot-potato fix, live-game
// Finding 2); everything else — the desperation-acquisition mechanism, valuation,
// the dispatcher — is v29 VERBATIM. Exposed as `claudeV33Bot`.
//
// THE v33 FIX (strong-set hot-potato): v14 gated Offer C on `rivalCanAcquire` from
// the CURRENT HOLDER, which closed the WEAK-set ring (a poor rival can't pry a
// cheap set loose) but left the STRONG-set ring open. A live 4-player game showed
// it: a cash-rich rival (Kyle) one dark-blue short, with the completer (Boardwalk)
// at a non-rival bot holdout. v14's gate PASSES (the rich rival genuinely could
// extract Boardwalk), so each bot re-books the full DENY premium and the lot
// HOT-POTATOES bot→bot for turns, net-zero cash, until the rival simply buys in
// anyway ($500). The miss: v14 priced the ABSOLUTE denial ("rival lacks the set")
// instead of the MARGINAL one ("does MY buy make it less reachable?"). Against a
// rival who can re-extract the completer from ANY firm holder, moving it holder→me
// changes nothing — the holder already blocks as well as I would. v33 adds the
// destination check: fire only when the rival can acquire from the holder but NOT
// from me after the buy — i.e. the holder is SOFTER than me (a distressed seat that
// would shed the completer cheap), so taking it into firm hands genuinely lifts the
// rival's cost out of reach. See the gate at Offer C.
//
...
export function proposeBestTrade(
  ...
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      ...
      // v33 MARGINAL-DENIAL GATE (live-game Finding 2, the STRONG-set hot-potato):
      // v14 closed the weak-set ring but NOT the strong one. A denial buy has value
      // only if it actually makes the completer UNACQUIRABLE to the rival. If the
      // rival could just buy it back from ME at my (firm, non-distressed)
      // threat-adjusted price, the denial is futile churn — the current holder
      // already blocks the rival exactly as well as I would, so moving the lot
      // holder→me changes nothing. That is the observed dark-blue ring: a cash-rich
      // rival can re-extract the completer from whoever holds it, so each bot
      // re-books the premium and passes it around in a circle until the rival buys
      // in anyway. The marginal denial is real ONLY when the current holder is
      // SOFTER than me — a distressed seat whose threat premium is discounted, so it
      // would shed the completer to the rival cheap — and taking it into firm hands
      // lifts the rival's cost out of reach. Fire iff the rival can acquire from the
      // holder but NOT from me after the buy.
      const afterIBuy = postTradeState(state, {
        propertyTo: { [pos]: pid },
        gojfTo: {},
        cashDelta: {},
      });
      if (rivalCanAcquire(afterIBuy, opp.id, pos, pid)) continue;
