...
import { hasMonopoly, ownablePrice } from "../../../logic";
...
// ===========================================================================
// v7 SNAPSHOT — self-contained fork of v5's `trades.ts` (see EVOLUTION.md).
//
// INHERITED (v5): trade-to-deny. When a rival is ONE lot short of a set and the
// completer sits with a third-party holdout, buy that lot to ME purely to block the
// completion, credited the `DENY_FACTOR` premium on the rival set. The denied rival
// is not a party, so it can't veto its own denial — the negative-sum asymmetry that
// transferred win share where v3/v4's positive-sum self-improvements washed out.
// `evaluateTrade` is unchanged; the denial credit lives only in the proposer's
// go/no-go (gate = plainDelta + DENY_FACTOR×bonus > ACCEPT_MIN).
//
// v7 HYPOTHESIS: "denying EARLIER catches prize sets v5's one-short denial can't."
// v6 (deny-via-swap) showed funding *reach* isn't the lever — cash-fundable
// one-short denials already capture that value. The other axis is SCOPE: block a
// rival while they're still TWO lots short. Holding ONE of the two lots they lack
// makes the set impossible just as surely as taking the last one — and at two-short
// the lots are still distributed across holdouts (and cheap), whereas by the time
// the rival is one-short they have often acquired the completer themselves, putting
// it out of trade-reach. So early denial converts blocks v5 simply misses.
//
// THE CHANGE (Offer D, isolated to construction): for each rival two lots short of a
// set, take the cheapest of its two missing lots that sits with a holdout. The
// credit is DISCOUNTED by `TWO_SHORT_DISCOUNT` because a two-short rival is further
// from completing (more uncertain to realize, and v5 could still block it later at
// one-short) — so weak sets self-gate even harder than at one-short, and a genuine
// completion or a one-short block always outranks an early block of equal raw value.
// `evaluateTrade` stays untouched; the early-denial credit lives only in the
// proposer's go/no-go, exactly like v5's one-short credit.
// ---------------------------------------------------------------------------
...
/** v7 — how much an EARLY (two-short) block is worth relative to a one-short block,
 *  as a fraction of the full `DENY_FACTOR` denial credit. Below 1 because a
 *  two-short rival is further from the monopoly: more uncertain to ever complete,
 *  and v5's one-short denial could still catch it later. A conservative haircut, so
 *  early denial fires only for genuinely strong sets and never outranks a real
 *  completion or an imminent one-short block. */
const TWO_SHORT_DISCOUNT = 0.5;
 ...
 *  which closes the 1-1-1 / two-short boards no 2-way deal can; per rival one lot
 *  short of a set whose completer sits with a third-party holdout, a denial buy of
 *  that lot (Offer C); and (v7) per rival TWO lots short, an EARLY denial buy of one
 *  of the two missing lots from a holdout (Offer D, discounted). At most one attempt
 *  per turn group; the highest denial-augmented delta wins. */
export function proposeBestTrade(
  ...
  // Offer D — EARLY (two-short) denial (v7). For each rival TWO lots short of a set,
  // take ONE of the two lots they lack from a third-party holdout — holding one
  // needed lot makes the set impossible for them, the same block as Offer C but
  // sprung a step earlier, while the lots are still distributed and cheap (by the
  // time the rival is one-short they have often grabbed the completer themselves).
  // The credit is discounted (`TWO_SHORT_DISCOUNT`) for the extra distance, so only
  // strong sets clear and a real completion / one-short block always outranks it.
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      const positions = groupPositions(color);
      // Two-short only makes sense for 3-lot color sets: there it means the rival
      // owns one and needs two. For a 2-lot set (brown, dark-blue) "two short" is
      // "owns none" — not a near-threat, and handled at one-short by Offer C — so
      // skip it here.
      if (positions.length < 3) continue;
      if (ownedInColor(state, opp.id, color) !== positions.length - 2) continue;
      const missing = positions.filter((pos) => state.ownership[pos] !== opp.id);
      // If I already hold one of the lots they lack, the set is already blocked.
      if (missing.some((pos) => state.ownership[pos] === pid)) continue;
      // Pick the cheapest of the missing lots held by a third-party holdout,
      // building-free — the least-cost way to make the set impossible.
      let target: number | undefined;
      for (const pos of missing) {
        const holder = state.ownership[pos];
        if (!holder || holder === pid || holder === opp.id) continue;
        if (builtLotsInGroup(pos, (p) => developmentLevel(state, p)).length > 0) continue;
        if (target === undefined || (ownablePrice(pos) ?? 0) < (ownablePrice(target) ?? 0)) {
          target = pos;
        }
      }
      if (target === undefined) continue;
      const holder = state.ownership[target];
      if (!holder) continue; // narrowing — target was chosen from owned-by-holdout lots
      const buy = sweetenFor(state, pid, holder, {
        propertyTo: { [target]: pid },
        gojfTo: {},
        cashDelta: {},
      });
      if (buy === null) continue;
      ...
      candidates.push({
        terms: buy,
        delta: 0,
        denyBonus: Math.round(monopolyBonus(color) * DENY_FACTOR * TWO_SHORT_DISCOUNT),
        reason: `Buying ${spaceName(target)} from ${holderName} to deny ${opp.name} the ${colorName(color)} early — they're two short.`,
      });
    }
  }
