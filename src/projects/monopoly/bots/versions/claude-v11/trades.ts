...
// ===========================================================================
// v11 SNAPSHOT — self-contained fork of v5's `trades.ts` (see EVOLUTION.md).
//
// v11 HYPOTHESIS (THREAT-WEIGHTED DENIAL): v5's denial premium is the same no
// matter WHICH rival is blocked — it would spend as much to deny a hopeless
// trailer as the player about to win. Denying a set off a rival who's going to
// lose anyway just wastes the holdout's sweetener; the dangerous completion is the
// LEADER's. v11 scales the denial premium by `threatWeight` — the denied rival's
// position value relative to the STRONGEST opponent — so denial concentrates on
// the genuine threat and the weakest trailer blocks self-prune. The weight is
// CAPPED at 1.0 (the leading opponent gets exactly v5's premium, never more — v10
// proved paying *above* v5's level overpays) and FLOORED (a real one-short block
// is worth something even against a laggard), so v11's denial spend is always ≤
// v5's: it reallocates and trims, never inflates. This is the one denial axis v5
// is naive about — target SELECTION — distinct from funding (v6), scope (v7), and
// price (v10), all of which failed.
//
// Everything below the threat scaling is v5 VERBATIM:
//
// v5 HYPOTHESIS: "denying a rival's prize set before they complete it transfers
// win share." v3 (complete sets sooner) and v4 (develop sooner) both came back
// WIN-NEUTRAL even against opponents lacking the capability, because a positive-sum
// self-improvement washes out symmetrically. The untested lever is a NEGATIVE-SUM
// move against a SPECIFIC rival — denial — which the rival can't neutralize by
// doing the same to someone else.
//
// THE CHANGE (isolated to construction): when a rival is one lot short of a set
// and that completer lot sits with a THIRD-PARTY holdout (not me, not the rival,
// building-free), construct a trade BUYING that lot to ME purely to DENY — even
// though it doesn't complete my own set. Its worth to me is the `DENY_FACTOR`
// premium on the rival's set I'm preventing — the SAME credit `acquisitionValue`
// already applies when I take a rival's last lot on landing/auction, which trade
// construction never did. The denied rival is NOT a party to the deal (only the
// holdout and I are), so it cannot veto its own denial — that is the asymmetry.
//
// `evaluateTrade` is UNCHANGED, so the inherited completion search, the
// counterparty model, and the incoming-offer vote behave exactly as in v3. The
// denial credit is added ONLY to the proposer's go/no-go on the new denial
// candidates (their own accept gate = plainDelta + DENY_FACTOR×bonus > ACCEPT_MIN);
// the holdout still judges the deal by plain `evaluateTrade`. Weak sets self-gate:
// a small bonus rarely overcomes the cash sweetener a holdout demands.
// ---------------------------------------------------------------------------
...
/** v11 THREAT-WEIGHTED DENIAL. The least a denial premium can be discounted to for
 *  a far-behind rival. A rival's denial weight is its position value over the
 *  strongest opponent's, clamped to [DENY_THREAT_FLOOR, 1]: the leading opponent
 *  always gets v5's full premium (cap 1.0 — v10 proved going above it overpays),
 *  while a laggard's block is trimmed toward this floor (never to zero — a
 *  one-short block still has real value against anyone). At 0.5 the discount is a
 *  deliberate tilt, not a purge: it reallocates denial toward the threat. */
const DENY_THREAT_FLOOR = 0.5;

/** How much `opp`'s monopoly threat is worth denying RIGHT NOW relative to the
 *  field: its position value as a fraction of the strongest active opponent's, in
 *  [DENY_THREAT_FLOOR, 1]. The leading opponent → 1.0 (full v5 premium); trailers
 *  → discounted. Used to scale the denial bonus so the bot concentrates its
 *  (cash-costly) blocks on the rival most able to use the set. */
export function threatWeight(state: GameState, pid: string, opp: string): number {
  const oppValue = positionValue(state, opp);
  let strongest = 0;
  for (const rival of activeOpponents(state, pid)) {
    strongest = Math.max(strongest, positionValue(state, rival.id));
  }
  if (strongest <= 0) return 1;
  const ratio = oppValue / strongest;
  return Math.min(1, Math.max(DENY_THREAT_FLOOR, ratio));
}
  ...
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      ...
      candidates.push({
        terms: buy,
        delta: 0,
        // v11: scale the denial premium by how strong this rival is relative to
        // the field — block the leader at full value, trim a laggard's block.
        denyBonus: Math.round(monopolyBonus(color) * DENY_FACTOR * threatWeight(state, pid, opp.id)),
